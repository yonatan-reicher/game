...
module Enemy =
    let initAt pos = { Position = pos; ShootDelay = 5f<s> }
    ...
    let private moveToAimAt (ft: Time.Frame) targetPos enemy =
        let desiredDis = 4f<m>
        let speed = 4f<m/s>
    ...
    let private shootIfCan enemy =
        if enemy.ShootDelay > 0f<s> then enemy else
            // Shoot
            printfn "Shoot!"
    ...
    let private decreaseDelay (ft: Time.Frame) enemy =
        { enemy with ShootDelay = enemy.ShootDelay - ft.Delta }


    let tick (ft: Time.Frame) targetPos enemy =
        enemy
        |> moveToAimAt ft targetPos
        |> shootIfCan
        |> decreaseDelay ft
